...
const slsa = require('@github/slsa-provenance')
...
const { isRegistryProvenanceEnabled } = require('../utils/provenance.js')
...
class Publish extends BaseCommand {
  ...
  async exec (args) {
    ...
    const provenanceDestination = this.npm.config.get('provenance-destination')
    ...
    if (await this.provenanceEnabled(registry, resolved.name)) {
      const digest = pkgContents.integrity.sha512[0].digest
      const subject = {
        name: pkgContents.filename,
        algorithm: 'sha512',
        digest: Buffer.from(digest, 'base64').toString('hex'),
      }

      const bundle = await slsa.generateProvenance(subject)

      await sendToTMA(pkgContents.id, bundle)
      log.notice('', `Publishing provenance`)

      if (provenanceDestination) {
        await writeFile(provenanceDestination,
          JSON.stringify({ ...bundle, mediaType: 'application/vnd.dev.sigstore.bundle.v1+json' })
        )
      }
    }
  ...
  async provenanceEnabled (registry, pkgName) {
    ...
    return (
      provenance &&
      !dryRun &&
      isRegistryProvenanceEnabled(registry) &&
      await this.isPackagePublic(pkgName)
    )
  }

  async isPackagePublic (name) {
    return npmFetch(name, { method: 'HEAD', forceAuth: {} })
      .then(() => true)
      .catch(() => false)
  }
}

async function sendToTMA (name, bundle) {
  const url = `https://trust-metadata-api-production.service.iad.github.net/attestations/pkg%3Anpm%2f${encodeURIComponent(name)}`

  const response = await fetch(url, {
    method: 'POST',
    body: JSON.stringify({ ...bundle, mediaType: 'application/vnd.dev.sigstore.bundle.v1+json' }),
